...
class Entries {
  ...
  async updateEntries (entriesToUpdate, space) {
    const updatedEntries = await asyncMap(entriesToUpdate, async (entry) => {
      try {
    ...
    return await asyncForEach(updatedEntries, async (entry) => {
      try {
        ...
        console.error('Error publishing updated Entry. Entry was:');
        console.log(JSON.stringify(entry));
        throw e;
      }
    });
  ...
  async createEntries (newEntries, space) {
    const createdEntries = await asyncMap(newEntries, async (entry) => {
      try {
        return await space.createEntryWithId(entry.sys.contentType.sys.id, entry.sys.id, entry);
      } catch(e) {
    ...
    return await asyncForEach(createdEntries, async (entry) => {
      try {
        return await entry.publish();
      } catch(e) {
