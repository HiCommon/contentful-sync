...
class ContentTypes {
  ...
  async removeOutdatedFields(originalContentType, updatedContentType) {
    ...
    console.log('updating with omitted')
    return await originalContentType.update();
  }
  async updateContentTypes (updatedContentTypes, space) {
    return await asyncForEach(updatedContentTypes, async (contentType) => {
      try {
        ...
        Object.assign(contentTypeWithRemovedFields, contentType);
        const updatedContentType = await contentTypeWithRemovedFields.update();
